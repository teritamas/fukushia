...
import {
  ...
  AdvancedSuggestedResource,
} from "../lib/advancedSuggestions";
...
export default function ClientResources({
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      try {
        ...
        setError(e instanceof Error ? e.message : "利用中資源の取得に失敗");
      } finally {
  ...
  useEffect(() => {
    const fetchRes = async () => {
      ...
      try {
        ...
        setError(e instanceof Error ? e.message : "社会資源一覧取得失敗");
      } finally {
  ...
  const suggestions: SuggestionEntry[] = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      return (
        (b.resource.last_verified_at || 0) - (a.resource.last_verified_at || 0)
      );
    });
  ...
  useEffect(() => {
    let cancelled = false;
    if (!assessmentData || !clientName) {
      setAdv(null);
      return;
    }
    setAdvLoading(true);
    setAdvFailed(false);
    fetchAdvancedSuggestions(assessmentData)
      .then((r) => {
        if (cancelled) return;
        if (r && r.resources) {
          setAdv(r.resources);
  ...
  const addUsage = async (res: ResourceRecord) => {
    ...
    try {
      ...
      setAddingIds((prev) => ({ ...prev, [res.id!]: false }));
    }
  };
  ...
  const addMemoInDetail = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/resources/${detailId}/memos`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ resource_id: detailId, content }),
      });
      const data = await res.json();
  ...
  const toggleStatus = async (u: ResourceUsageDoc) => {
    ...
    try {
      ...
      setTogglingIds((prev) => ({ ...prev, [u.id]: false }));
    }
  };
  ...
  const removeUsage = async (u: ResourceUsageDoc) => {
    ...
    try {
      ...
      setRemovingIds((prev) => ({ ...prev, [u.id]: false }));
    }
  };

  return (
    <div className="surface card-shadow border border-gray-100 rounded-xl p-4 sm:p-5 space-y-5">
      <h3 className="font-bold section-title mb-2">
        社会資源・制度の提案と利用状況
      </h3>
      {error && <div className="text-xs text-red-600">{error}</div>}
      {!clientName && (
        <p className="text-xs text-gray-500">支援対象者を選択してください。</p>
      )}
      {clientName && (
        <>
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-1 flex items-center gap-2">
              利用中の社会資源・制度{" "}
              {loadingUsage && (
                <span className="text-[10px] text-gray-400">読込中...</span>
              )}
            </h4>
            {loadingUsage ? (
              <div className="space-y-2">
                {[...Array(3)].map((_, i) => (
                  <div
                    key={i}
                    className="surface card-shadow border border-gray-100 rounded-lg p-3 animate-pulse"
                  >
            ...
            <ul className="flex flex-col gap-2">
              {usages.map((u) => (
                <li
                  key={u.id}
                  className="surface border border-gray-100 rounded-lg p-2 flex flex-col sm:flex-row sm:items-center gap-2 text-xs"
                >
                  <div className="flex-1 flex items-center gap-2">
                    <button
                      onClick={() => openDetail(u.resourceId)}
                      className="font-semibold mr-1 underline decoration-dotted hover:text-[var(--brand-600)] text-left"
                    >
                      {u.serviceName}
                    </button>
                    <span
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button
                      ...
                      className="gbtn text text-[11px] h-8"
                    >
                      {u.status === "active" ? "終了" : "再開"}
                    </button>
                    <button
                      onClick={() => removeUsage(u)}
                      disabled={removingIds[u.id]}
                      className="gbtn text text-[11px] h-8 text-red-600"
                    >
                      削除
                    </button>
                  </div>
          ...
          {assessmentData ? (
            <div>
              ...
              {advFailed && !advLoading && (
                <div className="text-[11px] text-red-600 flex flex-col gap-1">
                  <span>
                    AI提案の取得に失敗しました（ネットワークまたはサーバエラー）。
                  </span>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={retryAdvanced}
                      className="gbtn primary text-[11px] h-8 bg-red-600 hover:bg-red-700"
                    >
                      再試行
                    </button>
                    <span className="text-[10px] text-gray-500">
                      フォールバック: キーワード一致のみを表示
                    </span>
                  </div>
                </div>
              )}
              {loadingResources || advLoading ? (
                ...
                suggestions.length === 0 && (
                  <p className="text-[11px] text-gray-500">
                    現在のアセスメント内容から提案できる社会資源・制度は見つかりませんでした。記述を具体化するか関連キーワードを追加してください。
                  </p>
                )
              )}
            ...
            <div>
              ...
              {hasAssessmentPlan ? (
                <p className="text-[11px] text-gray-500">
                  アセスメントはありますがテキスト化できる内容がほとんど無いため提案を生成できません。各カテゴリに要約や所感を入力後、対象者を再選択してください。
                </p>
              ) : (
                <p className="text-[11px] text-gray-500">
                  最新アセスメントが保存されていないため提案は表示されません。まずアセスメントを作成してください。
                </p>
              )}
            </div>
          )}
                ...
                <div className="pt-3">
                  ...
                  <div className="max-h-48 overflow-auto pr-1">
                    <ul className="space-y-1">
                      {(detailId && memos[detailId] ? memos[detailId] : []).map(
                        (m) => (
                          <li
                            key={m.id}
