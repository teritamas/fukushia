        ...
        {currentClient && (
          <div className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-4 items-start">
            ...
            <div className="md:col-span-3 space-y-2">
              <div className="flex items-center gap-2">
                <h3 className="text-xs font-semibold text-gray-600 flex items-center gap-2">
                  ...
                  {hasAssessment && (
                    <button
                      onClick={() => {
                        requestAssessmentEdit({ category: "本人情報" });
                ...
                {changedKeys.size > 0 && (
                  <span className="text-[10px] bg-amber-200 text-amber-800 px-2 py-0.5 rounded">
                    {changedKeys.size}項目更新
                  </span>
                )}
              </div>
              {personalLoading && (
              ...
              {personalError && (
                <p className="text-[11px] text-red-500">{personalError}</p>
              )}
              {!personalLoading &&
                !personalError &&
                (hasAssessment && Object.values(personalInfo).some((v) => v) ? (
                      ...
                      {[
                        ...
                        return (
                          <div key={label} className={rowBg}>
                            <div className="flex">
                              <div className="text-left px-2 py-1 font-medium w-24 text-gray-600">
                                {label}
                              </div>
                  ...
                  <div className="p-3 border rounded bg-gray-50 text-[11px] flex flex-col sm:flex-row sm:items-center gap-3">
                    <div className="flex-1 text-gray-600">
                      まだアセスメント（本人情報）が登録されていません。AI
                      を用いて初回アセスメントを作成できます。
                    </div>
                    <button
                      onClick={() => setActiveTab("assessment")}
                      className="bg-indigo-600 text-white text-xs px-3 py-1.5 rounded shadow hover:bg-indigo-700 whitespace-nowrap"
                    >
                      AI で初回アセスメント作成
                    </button>
                  </div>
