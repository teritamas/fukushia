...
export default function ResourceManager() {
  ...
  const addMemoInDetail = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/resources/${detailResource.id}/memos`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const updateMemo = async (memo: ResourceMemo) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/resources/memos/${memo.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-6">
      ...
      <div className="surface card-shadow border border-[var(--border)] p-3 sm:p-4 flex flex-wrap gap-2 items-center">
        ...
        <button
          ...
          disabled={searching}
          className="gbtn primary disabled:opacity-60 text-sm"
        >
          検索
        </button>
        {searchMode && (
        ...
        >
          {showEditor ? "フォームを閉じる" : "追加/編集フォームを開く"}
        </button>
      </div>
      {showEditor && (
        <div className="surface card-shadow border border-[var(--border)] rounded-lg p-4 sm:p-5 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            />
            <input
              className="ginput sm:col-span-2"
              placeholder="キーワード (空白/カンマ区切り)"
            ...
            <textarea
              ...
              onChange={(e) =>
                handleChange("application_process", e.target.value)
              }
            />
          </div>
          <div className="flex gap-2 flex-wrap items-center">
            <button
              ...
              disabled={!form.service_name.trim() || actionLoading}
            ...
            <button
              onClick={() => handleImportLocal(false)}
              disabled={actionLoading}
              className="gbtn tonal disabled:opacity-60"
            >
              Import
            </button>
            <button
              onClick={() => handleImportLocal(true)}
              disabled={actionLoading}
              className="gbtn disabled:opacity-60 bg-red-600 text-white border-transparent hover:bg-red-700"
            >
              Import上書き
            </button>
          ...
          {importResult && (
            <div className="text-xs bg-[var(--surface)] border border-[var(--border)] rounded p-2 space-y-1">
              ...
              <div>
                total: {importResult.total_input} / created:{" "}
      ...
      <div>
        <h3 className="font-semibold mb-2 section-title">
          登録済み ({resources.length})
        </h3>
        {resourcesLoading ? (
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="font-bold text-sm leading-snug break-words flex-1">
                      {r.service_name}
                    </div>
                    <span
                      className={`chip ${
                ...
                <div className="pt-3">
                  ...
                  <div className="max-h-48 overflow-auto pr-1">
                    <ul className="space-y-1">
                      ...
                      {(memos[detailResource.id!] || []).length === 0 &&
                        !memoLoading[detailResource.id!] && (
