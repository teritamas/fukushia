...
export default function AppHeader({ active, onChange }: AppHeaderProps) {
  const { currentClient, setCurrentClient, requestGoToBasicInfo } = useClientContext();
    ...
    <header className="w-full sticky top-0 z-30 bg-gradient-to-r from-blue-700 to-blue-800 text-white header-shadow">
      <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col md:flex-row md:items-center gap-3">
        <button
          ...
          onClick={()=>{ onChange('clients'); requestGoToBasicInfo(); }}
          title="ダッシュボードに戻る"
        >福祉支援ダッシュボード</button>
        <div className="ml-auto flex items-center gap-3">
          <nav className="flex flex-wrap items-center gap-2">
            ...
            >
              <span className="truncate max-w-[170px]">{currentClient?.name || (loadingClients ? '読み込み中...' : '支援対象者を選択')}</span>
        ...
        {showAdd && typeof window !== 'undefined' && createPortal(
          ...
          >
            ...
            >
              <h3 className="text-base font-semibold mb-3 section-title">新しい支援対象者を追加</h3>
              ...
              <div className="mt-4 flex justify-end gap-2 text-sm">
                <button onClick={()=> { setShowAdd(false); setNewName(''); setNewPhotoUrl(''); }} className="gbtn text">キャンセル</button>
                <button onClick={handleAddClient} disabled={adding || !newName.trim()} className="gbtn primary disabled:opacity-60">{adding ? '追加中...' : '追加'}</button>
              </div>
            </div>
          </div>,
          document.body
