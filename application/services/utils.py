...
def relative_date_tool(text, base_ts):
    ...
    patterns = [
        (r"来週月曜日", lambda d: d + timedelta(days=(7 - d.weekday() + 0) % 7 + 7)),
        (r"来週火曜日", lambda d: d + timedelta(days=(7 - d.weekday() + 1) % 7 + 7)),
        (r"来週水曜日", lambda d: d + timedelta(days=(7 - d.weekday() + 2) % 7 + 7)),
        (r"来週木曜日", lambda d: d + timedelta(days=(7 - d.weekday() + 3) % 7 + 7)),
        (r"来週金曜日", lambda d: d + timedelta(days=(7 - d.weekday() + 4) % 7 + 7)),
        (r"来週土曜日", lambda d: d + timedelta(days=(7 - d.weekday() + 5) % 7 + 7)),
        (r"来週日曜日", lambda d: d + timedelta(days=(7 - d.weekday() + 6) % 7 + 7)),
